...
for prsn in ["SG", "SM"]:
  ...
  df_cntry = (
    pl
    .read_excel(f"an_llm/data/handcoding/done/data_filtered_sample_ger_{prsn}.xlsx")
...
df_ai = pl.read_excel("an_llm/data/data_results_v5_ger.xlsx")
...
df_combined = (
  ...
  .with_columns(
      ...
      match_LLM_SM = pl.col("token_1") == pl.col("concept_SM"),
      match_SG_SM = pl.col("concept_SG") == pl.col("concept_SM")
  )
  .select(
    ["token_1", "concept_SG", "concept_SM", "match_SG_SM", "match_LLM_SG", "match_LLM_SM", "text", "url"]
  )
)


df_combined.write_excel("an_llm/data/handcoding/results/data_sample_results_combined_v5_ger.xlsx")
...
match_token1_conceptYS = compute_match_percentage(df_combined, "token_1", "concept_SM")
match_conceptSG_conceptYS = compute_match_percentage(df_combined, "concept_SG", "concept_SM")

# Print results
print(f"token_1 vs concept_SG: {match_token1_conceptSG:.2f}% match")
print(f"token_1 vs concept_SM: {match_token1_conceptYS:.2f}% match")
print(f"concept_SG vs concept_SM: {match_conceptSG_conceptYS:.2f}% match")


# draw 3 scatter plots for each inter-coder pair (dis)agreement of categories
import matplotlib.pyplot as plt

pairs = [("token_1", "concept_SG"), ("token_1", "concept_SM"), ("concept_SG", "concept_SM")]

df_dropped = df_combined.drop_nulls(["token_1", "concept_SG", "concept_SM"])
...
plt.savefig("an_llm/data/handcoding/results/plot_scatter_rater_v5_ger.png", dpi=300, bbox_inches='tight')
