...
# Number of rows: 995494
# Number of unique urls: 101087
# Number of unique domains: 1173
# Mean rows per url: 9.85


# 1. filter polars dataframe text elements by diversity keywords

# keyword list
with open("data/keywords_strict.txt", "r", encoding="utf-8") as f:
    keywords_strict = [line.strip() for line in f if line.strip()]

with open("data/keywords_compound.txt", "r", encoding="utf-8") as f:
    keywords_compound = [line.strip() for line in f if line.strip()]

pattern_strict = r"\b(?:" + "|".join(keywords_strict) + r")\b"
pattern_compound = r"\w*(?:" + "|".join(keywords_compound) + r")\w*"

pattern = f"(?i){pattern_strict}|{pattern_compound}"
...
# Number of rows: 7429 6610 (5840)
# Number of unique urls: 4195 3758 (3367)
# Number of unique domains: 789 764 (721)
# Mean rows per url: 1.77 1.76 (1.73)
